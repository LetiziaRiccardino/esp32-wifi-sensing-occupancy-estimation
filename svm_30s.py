...
# Disabilita avvisi minori di pandas per mantenere il report pulito
warnings.filterwarnings('ignore')

# 1. CARICAMENTO FILE E PULIZIA DATI
...
# Rimozione record nulli nelle colonne vitali
...
# 2. SEGMENTAZIONE DELLE FINESTRE TEMPORALI REALI
...
# Rilevamento automatico delle sessioni di test (separati da pause > 5 minuti)
...
def calcola_features_finestra(group):
    ...
    # Feature statistiche  MOTION INDEX
...
# Controllo di stabilità sulle classi per lo split stratificato
...
print("\nEsecuzione Grid Search per SUPPORT VECTOR MACHINE (SVM)...")

# Pipeline per separare addestramento e test
...
grid_svm = GridSearchCV(estimator=pipeline_svm, param_grid=param_grid_svm, cv=cv_strategy, scoring='accuracy', n_jobs=-1)
grid_svm.fit(X_train, y_train)

# Estrazione del modello ottimo 
best_svm = grid_svm.best_estimator_
y_pred_svm = best_svm.predict(X_test)

# Importanza delle feature
print("Calcolo della Permutation Feature Importance per il confronto delle feature...")
risultato_permutazione = permutation_importance(
    best_svm, X_test, y_test, n_repeats=10, random_state=42, n_jobs=-1
)
...
with contextlib.redirect_stdout(output_buffer):
    print("=" * 85)
    print(f"      REPORT: SUPPORT VECTOR MACHINE (SVM) - FINESTRE {DURATA_FINESTRA}s")
    print("=" * 85)
    ...
    print(f"  - Migliore configurazione di parametri trovata: {grid_svm.best_params_}")
    print(f"  - Accuratezza media stimata in Cross-Validation : {grid_svm.best_score_:.4f}")
    
    print("\n[3] PERFORMANCE SUL TEST SET GENERALE (DATI MAI VISTI):")
    acc_svm = accuracy_score(y_test, y_pred_svm)
    print(f"  - Accuracy Score complessiva dell'SVM: {acc_svm:.4f} ({acc_svm*100:.2f}%)")
    
    print("\n[4] METRICHE DI CLASSIFICAZIONE ANALITICHE (PRECISION, RECALL, F1):")
    print(classification_report(y_test, y_pred_svm))
    
    print("[5] MATRICE DI CONFUSIONE:")
    cm_svm = confusion_matrix(y_test, y_pred_svm)
    classes_labels = sorted(y.unique())
    header_pred = " ".join([f"Predetto {c}".center(14) for c in classes_labels])
    print(f"                {header_pred}")
    for idx, row in enumerate(cm_svm):
        row_str = " ".join([f"{val}".center(14) for val in row])
        print(f"  Reale {classes_labels[idx]} pers. :    {row_str}")
    
    print("\n[6] GERARCHIA DI IMPORTANZA DELLE FEATURE (FEATURE IMPORTANCE BASATA SU PERMUTAZIONE):")
    importances_svm = pd.Series(risultato_permutazione.importances_mean, index=X.columns).sort_values(ascending=False)
    for index, (feat, imp) in enumerate(importances_svm.items(), start=1):
        valore_imp = max(0.0, imp)  # Elimina micro-oscillazioni negative matematiche sotto lo zero
        print(f"  {index:02d}. {feat:<22}: {valore_imp:.4f} ({valore_imp*100:.2f}%)")
    print("=" * 85)

#Scrittura del file
nome_file_report = 'report_dettagliato_svm.txt'
with open(nome_file_report, 'w', encoding='utf-8') as f:
    f.write(output_buffer.getvalue())

print(f"\n[COMPLETATO] Report generato con successo.")
