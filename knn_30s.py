...
# 3. ESTRAZIONE DELLE FEATURE AGGREGATE 
...
# 5. ADDESTRAMENTO E GRID SEARCH: K-NEAREST NEIGHBORS (KNN)

print("\nEsecuzione Grid Search per K-NEAREST NEIGHBORS (KNN)...")
...
#  iperparametri per KNN (Numero di vicini, metrica di distanza e peso)
param_grid_knn = {
...
grid_knn = GridSearchCV(estimator=pipeline_knn, param_grid=param_grid_knn, cv=cv_strategy, scoring='accuracy', n_jobs=-1)
...
# Calcolo della Permutation Feature Importance
print("Calcolo della Permutation Feature Importance ...")
...
with contextlib.redirect_stdout(output_buffer):
    print("=" * 85)
    print(f"      REPORT: K-NEAREST NEIGHBORS (KNN) - FINESTRE {DURATA_FINESTRA}s")
    print("=" * 85)
    ...
    print(f"  - Migliore configurazione di parametri trovata: {grid_knn.best_params_}")
    print(f"  - Accuratezza media stimata in Cross-Validation : {grid_knn.best_score_:.4f}")
    
    print("\n[3] PERFORMANCE SUL TEST SET GENERALE (DATI MAI VISTI):")
    acc_knn = accuracy_score(y_test, y_pred_knn)
    print(f"  - Accuracy Score complessiva del KNN: {acc_knn:.4f} ({acc_knn*100:.2f}%)")
    ...
    print(f"                {header_pred}")
    for idx, row in enumerate(cm_knn):
    ...
    importances_knn = pd.Series(risultato_permutazione.importances_mean, index=X.columns).sort_values(ascending=False)
    for index, (feat, imp) in enumerate(importances_knn.items(), start=1):
...
# Scrittura del file
nome_file_report = 'report_dettagliato_knn.txt'
with open(nome_file_report, 'w', encoding='utf-8') as f:
    f.write(output_buffer.getvalue())

print(f"\n[COMPLETATO] Report KNN generato con successo.")
